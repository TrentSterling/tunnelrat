...
//
// buildLevel(seed, config, depth, onProgress?) -> {
//   graph,                       // from generateLevelGraph
//   field,                       // from createDensityField
//   caveGroup: THREE.Group,      // all cave chunk meshes + decor group (decor.js) +
//                                // props group (props.js), ready to scene.add();
//                                // automap hologram + level disposal pick both up
//                                // automatically via traverse
...
//   biome,                       // getBiome(depth) from config.js; CALLER applies
//                                // biome.fog + biome.fogDensity to scene.fog and
//                                // scene.background after each build
// }
//
// SIGNATURE NOTE: depth is the 3rd parameter (drives biome selection + rock colors).
// Back-compat shim: buildLevel(seed, config, onProgressFn) still works (depth
// defaults to 1 = mine biome) so existing call sites keep running until rewired.
//
...
// 'color' attribute: vertex color = lerp(biome.rockDeep, biome.rockShallow,
...
import { getBiome } from '../config.js';
...
export function buildLevel(seed, config, depth, onProgress) {
  // back-compat shim: old call sites pass onProgress as the 3rd arg
  if (typeof depth === 'function') { onProgress = depth; depth = 1; }
  const cfg = config;
  const biome = getBiome(depth);
  const graph = generateLevelGraph(seed, cfg);
  const field = createDensityField(graph, seed, cfg, biome);
  ...
  const rockDeep = new THREE.Color(biome.rockDeep);
  const rockShallow = new THREE.Color(biome.rockShallow);
  ...
  // cave junk (props.js): mining leftovers + biome formations, same disposal regime
  caveGroup.add(buildProps(field, graph, cfg, biome, seed));
  ...
  return { graph, field, caveGroup, spawnPos, spawnQuat, biome };
}
