// verifier: spawn / exit / reactor / showcase screenshots via CDP
...
const PORT = 9600 + Math.floor((Date.now() / 1000) % 200);
const userDir = join(tmpdir(), 'tr-verify-' + PORT);
...
(async () => {
  ...
  // a: spawn view
  await shot('verify-spawn.png');

  // b: teleport('exit')
  await evl('TR.debug.teleport("exit"); true');
  await sleep(500);
  await shot('verify-exit.png');

  // c: teleport('reactor')
  await evl('TR.debug.teleport("reactor"); true');
  await sleep(500);
  await shot('verify-reactor.png');

  // d: showcase
  await evl('TR.debug.god(); TR.debug.showcase(); true');
  await sleep(800);
  await shot('verify-showcase.png');
